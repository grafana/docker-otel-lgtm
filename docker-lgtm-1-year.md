...
Observability in Under 5 Seconds: grafana/otel-lgtm Turns 1!

## Content

Happy Birthday to `grafana/otel-lgtm`! Over the past year, it has achieved 1k stars, celebrated version 0.11.1, and made observability accessible to everyone.
...
Wait for the log message `The OpenTelemetry collector and the Grafana LGTM stack are up and running.` or check for the `/tmp/ready` file. Then, start your [OpenTelemetry-exporting application](https://opentelemetry.io/docs/zero-code/). Examples are available in the [repository](https://github.com/grafana/docker-otel-lgtm/tree/main/examples).
...
`grafana/otel-lgtm` has made it easier for projects to create quickstart guides for observability integrations. Some notable adopters include:
...
Originally built for testing (e.g., [Testcontainers](https://testcontainers.com/modules/grafana/)), its ease of use has inspired some users to run it in production setups.
...
> *Note*: We also created a [OATs](https://github.com/grafana/oats), 
...
Here at Grafana Labs, we’re committed to OpenTelemetry and helping users advance their observability strategies. Try out [grafana/docker-otel-lgtm](https://github.com/grafana/docker-otel-lgtm/) today!
