...
git clone --filter=blob:none --no-checkout https://github.com/grafana/oats "$workdir/oats-src"
git -C "$workdir/oats-src" fetch --depth 1 origin 1febf4a
git -C "$workdir/oats-src" checkout FETCH_HEAD
