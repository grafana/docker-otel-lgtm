...
overlay="$TESTDIR/otelcol-config-export-http.yaml"
...
@test "debug only: overlay has no external exporters" {
	export OTEL_COLLECTOR_DEBUG_EXPORTER=true
	run run_otelcol
	! grep -q "otlphttp/external" "$TESTDIR/otelcol-config-export-http.yaml"
}
...
@test "external only: overlay has no debug exporters" {
	export OTEL_EXPORTER_OTLP_ENDPOINT=http://collector:4318
	run run_otelcol
	! grep -q "debug/" "$TESTDIR/otelcol-config-export-http.yaml"
}
